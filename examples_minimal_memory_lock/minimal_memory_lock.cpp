...
int prefault_dynamic_memory(size_t process_max_dynamic_memory)
{
  void * buf = nullptr;
...
int main(int argc, char * argv[])
{
  ...
  auto lock_memory_res = lock_memory();
  if (lock_memory_res != 0) {
    return EXIT_FAILURE;
  }

  auto prefault_res = prefault_dynamic_memory(process_max_dynamic_memory);
  if (prefault_res != 0) {
    return EXIT_FAILURE;
  }
